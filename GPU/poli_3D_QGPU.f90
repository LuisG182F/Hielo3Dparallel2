...
integer, parameter :: filas = N, columnas = 10, ancho = N, stoptime = 1003000
character(len=60), parameter :: radio_fijo = 'n'
integer :: p, posicion_local_idx, color, num_perms, num_pos
...
! Arreglos para guardar el orden aleatorio de las tareas
integer :: posicion_orden(0:7), color_orden(0:7)
integer :: rand_idx
real :: rand_num

integer, parameter :: NUM_PERMUTACIONES = 64
integer :: permutaciones_host(0:NUM_PERMUTACIONES-1, 0:7)
integer :: temp_perm(0:7)
...
do i=1,91
   angulote=1.0*i
   difa=Lif_CPU(angulote-1.0,energy,ang_l)
   write(45,*) i-1,difa
end do
...
! Generar la tabla de permutaciones aleatorias
    do p = 0, NUM_PERMUTACIONES-1
        ! Inicializar con 0,1,2,3,4,5,6,7
        do i = 0, 7
            temp_perm(i) = i
        end do
        ...
        do i = 7, 1, -1
            rand_idx = int(RandNew * (i + 1))
            temp = temp_perm(i)
            temp_perm(i) = temp_perm(rand_idx)
            temp_perm(rand_idx) = temp
        end do
        permutaciones_host(p, :) = temp_perm(:)
    end do
...
call inicializar_gpu(c, filas, columnas, ancho, permutaciones_host,BG_part0, BG_part1, BG_part2)
    
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!           aqui empieza el bucle de pasos
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
print*,time
    ...
    w=0
    cont_radio=0
 print*,'_______________________paso1__________________________'

!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!           aqui empieza el calculo de montecarlo
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

do posicion_local_idx = 0, 7 ! Índice del paso (0-7)
      do color = 0, 7
            call ejecutar_paso_montecarlo(filas, columnas, ancho, energy,&
...
call finalizar_gpu(c, BG_part0, BG_part1, BG_part2, time, paso, paso_area, paso_luz) ! Limpiar memoria de la GPU
...
! termina bucle de tiempo
call Fin_gpu()

close(19)
close(13)
deallocate(vect, orient1, c)
